...
Column {data-width=700}
-------------------------------------
### "Gemaakt voor dit" by Lil' Kleine & "Signs" by Snoop Dogg
```{r}
gemaakt_voor_dit <- 
...
grid.arrange(gemaakt_voor_dit_timbre, gemaakt_voor_dit_chroma, signs_timbre, signs_chroma, ncol=2, heights=c(2,2), top = "Timbre-based vs Chroma-based Self-Similarity matrices")
```
...
The first row shows the self-similarity matrices for "Gemaakt voor dit", where the first one is based on timbre, and the second one is based on chroma. The features were summarised at bar level and the axes are in seconds. The second row shows the same analysis for the song "Signs" by Snoop Dogg. In the matrices, we see a checkerboard structure, indicating a lot of repetitions. To make it easy to dance to a song, I think the song should be predictable and that can be accomplished by repetition. The chroma and timbre patterns for "Gemaakt voor dit" seem to fit nicely on top of each other, showing that changes in chroma go together with changes in timbre.
...
```{R}
ml_corpus <- rbind(sample_n(nederhop, 30), sample_n(american_hiphop, 30))
ml_data <- ml_corpus %>% add_audio_analysis %>% 
    mutate(
        segments = 
...
corpus_juice <- 
    ...
    prep(ml_data %>% mutate(track_name = str_trunc(track_name, 20))) %>% 
    juice %>% 
...
-------------------------------------
### Notes
I randomly selected 30 dutch songs and 30 american songs to perform machine learing on. To the left, we see an hierarchical clustering for the data. On the left we see mostly american songs and on the right we see mostly dutch songs. In the middle it is mixed.